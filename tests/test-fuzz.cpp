...
#ifdef NOFUZZ
// Main function that replicates the fuzzer behavior
int main(int argc, char* argv[]) {
    std::vector<uint8_t> file_data;

    if (argc > 1) {
        // Read model file from command line argument
        ...
        if (size < 32 || size > 100 * 1024 * 1024) {
            std::cerr << "File size out of bounds: " << size << " bytes" << std::endl;
            return 1;
        }
        ...
        std::cout << "Loaded model file: " << filename << " (" << size << " bytes)" << std::endl;
    } else {
        // Create minimal test data if no file specified
        std::cout << "No file specified, creating minimal test data..." << std::endl;

        // Create some dummy data that's at least 32 bytes
        file_data.resize(64);
    ...
    // Call the fuzzer function with our data
    std::cout << "Calling fuzzer function with " << file_data.size() << " bytes of data..." << std::endl;
    int result = LLVMFuzzerTestOneInput(file_data.data(), file_data.size());

    std::cout << "Fuzzer function returned: " << result << std::endl;
    return result;
}
#endif